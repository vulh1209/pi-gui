import type { BrowserHostAction, BrowserHostActionSequence, BrowserScrollTarget } from "./types";
...
export const BROWSER_COMMAND_USAGE = [
  "open <url>",
  "navigate <url>",
  "focus",
  "back",
...
export function parseBrowserCommandArgs(args: string): BrowserHostAction | undefined {
  ...
  if (verb === "open" || verb === "navigate") {
    const url = normalizeBrowserUrl(tokens.slice(1).join(" ").trim());
    return url ? { name: verb, url } : undefined;
  }
  ...
  if (verb === "type") {
    ...
    return selector && text ? { name: "type", selector, text } : undefined;
  }
  ...
  if (verb === "scroll") {
    const target = tokens[1]?.trim().toLowerCase();
    return isScrollTarget(target) ? { name: "scroll", target } : undefined;
  }
...
export function parseNaturalLanguageBrowserIntentSequence(text: string): BrowserHostActionSequence | undefined {
  const trimmed = text.trim();
  if (!trimmed) {
    return undefined;
  }
...
export function parsePreferredBrowserWebIntent(text: string): BrowserHostActionSequence | undefined {
  const trimmed = text.trim();
  if (!trimmed) {
    return undefined;
  }
...
function isScrollTarget(value: string | undefined): value is BrowserScrollTarget {
...
function buildBrowserSearchSequence(url: string, query: string): BrowserHostActionSequence {
  const selector = "textarea[name='q'], input[name='q'], input[type='search'], input[type='text'], textarea";
  return {
    label: `Browser search ${query}`,
    actions: [
      { name: "open", url },
      { name: "type", selector, text: query },
